...
<a href="/album/{album.id}" class="group block">
	<div class="mb-3 relative aspect-square overflow-hidden rounded-lg bg-surface-2">
		{#if album.coverThumb}
			<img
		...
		{/if}
		<div
			class="right-2 bottom-2 h-10 w-10 translate-y-2 shadow-lg group-hover:translate-y-0 absolute flex items-center justify-center rounded-full bg-accent opacity-0 transition-all duration-200 group-hover:opacity-100"
		>
	...
	<h3
		class="text-sm font-medium truncate text-text-primary transition-colors group-hover:text-accent"
	>
		{album.title}
	</h3>
